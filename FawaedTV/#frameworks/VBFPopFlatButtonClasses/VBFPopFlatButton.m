...
@property (nonatomic, strong) CAShapeLayer *layerLoading;
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        self.currentButtonType = buttonDownArrowType;
        self.currentButtonStyle = buttonPlainStyle;
        self.lineThickness = 2;
        self.lineRadius = 0;
        self.animateToStartPosition = NO;
        self.tintColor = [UIColor colorWithRed:0 green:122/255.0 blue:1.0 alpha:1.0];
        [self commonSetup];
    }
    return self;
}
...
- (void)animateToType:(FlatButtonType)finalType {
    self.layerLoading.opacity   = 0;
    self.layerProgress.opacity  = 0;
...
- (void)setProgress:(float)progress{
    ...
    if (!self.layerProgress) {
        self.layerProgress = [CAShapeLayer new];
        [self.layerProgress setStrokeColor:self.tintColor.CGColor];
        [self.layerProgress setFillColor:[UIColor clearColor].CGColor];
        self.layerProgress.bounds    = CGRectMake(0, 0, self.layer.bounds.size.width, self.layer.bounds.size.height);
        self.layerProgress.position  = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);;
        self.layerProgress.anchorPoint= CGPointMake(0.5, 0.5);
    }
    if (![self.layer.sublayers containsObject:self.layerProgress]) {
        [self.layer addSublayer:self.layerProgress];
    }
    UIBezierPath *processPath   = [UIBezierPath bezierPath];
    CGPoint center              = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
    CGFloat radius              = self.bounds.size.width*1.4 / 2.0;
...
- (void)addLoadingInidicator{
    ...
    if (!self.layerLoading) {
        self.layerLoading = [CAShapeLayer new];
        [self.layerLoading setStrokeColor:self.tintColor.CGColor];
        [self.layerLoading setFillColor:[UIColor clearColor].CGColor];
        
        UIBezierPath *processPath   = [UIBezierPath bezierPath];
        CGFloat radius              = (self.bounds.size.width - 1*3) / 2.0;
        CGPoint center              = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
        [processPath addArcWithCenter:center
                               radius:radius
                           startAngle:M_PI_2+M_PI_2/3
                             endAngle:M_PI_2-M_PI_2/3
                            clockwise:YES];
        [self.layerLoading setPath:processPath.CGPath];
        [self.layerLoading setOpacity:0];
        [self.layer addSublayer:self.layerLoading];
        
        self.layerLoading.lineWidth = 2;
        self.layerLoading.bounds    = CGRectMake(0, 0, self.layer.bounds.size.width, self.layer.bounds.size.height);
